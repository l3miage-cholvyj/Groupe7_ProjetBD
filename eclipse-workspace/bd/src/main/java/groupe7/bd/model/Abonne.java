...
public class Abonne {
	...
	public void loadAbonne(String nomAbonne, String codeSecretAbonne) {
		try {
			...
			//Sql commande
			String sqlCommand = "SELECT * FROM Abonne NATURAL JOIN Client WHERE (nom = '"+nomAbonne+"' and codeSecret =" +codeSecretAbonne+")";
	...
	public void SaveNewAbonne() {
		...
		try {
			...
			String sqlCommand = "INSERT INTO Client (numCB) VALUES ('"+this.numeroCarteBancaire+"')";
			Connection conn=TheConnection.getInstance();
			...
			sqlCommand = "INSERT INTO Abonne (idClient,nom,prenom,dateNaissance,sexe,dateAbonnement,codeSecret) VALUES ("
			+idClient+",'"+this.nom+"','"+this.prenom+"','"+dateDeNaissanceSQL+"','"+sexeVal+"','"+dateDAbonnementSQL+"','"+this.codeSecret+"')";
			stmt = conn.createStatement();
			//Statement stmt = conn.prepareStatement(sqlCommand, Statement.RETURN_GENERATED_KEYS);
			stmt.executeUpdate(sqlCommand);
			
			//retourne l' id de l'abonne
			sqlCommand = "SELECT MAX(idAbonne) AS id FROM Abonne;";
			requete = conn.createStatement();
	...
	public void showLocation() {
		...
		//Sql commande
		String sqlCommand = "SELECT * FROM Locations NATURAL JOIN Client LEFT JOIN Abonne USING (idClient) WHERE (fini = 0 and idAbonne = "+this.idAbonne+");";
		try {
			requete = conn.createStatement();
			ResultSet resultat = requete.executeQuery(sqlCommand);
