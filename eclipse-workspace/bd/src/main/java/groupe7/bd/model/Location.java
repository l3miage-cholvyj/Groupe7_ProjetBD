...
public class Location {
	...
	/*
	 * [Sql]
	 * Intteroge la base de donnèe pour rècupèrer les valeurs de la location
	 * "idLocations".
	 * [Java]
	...
	public void loadLocationIdLocations(int idLocations) {
		...
		try {
			...
			// Sql commande
			String sqlCommand = "SELECT * FROM Locations  WHERE (idLocations = " + idLocations + ")";
	...
	public void shownLocationIdLocations() {
		...
		System.out.println("***");
		for (LocationsDetail LocatationDetail : velos)
			LocatationDetail.ShowLocation();
		System.out.println("***");
	}
	 ...
	 * 
	 * [Sql]
	 * Met à jour la base de donnèe en créant une location associé à aucun abonné
	...
	public void editLocationAnonyme(String numCB, String codeSecret) {
		...
		// [Sql]
		try {
			String sqlCommand = "INSERT INTO Client (numCB) VALUES ('" + clientAnnonyme.getNumeroCarteBancaire() + "')";
			Connection conn = TheConnection.getInstance();
			...
			sqlCommand = "SELECT MAX(idClient) AS id FROM Client;";
			...
			sqlCommand = "INSERT INTO Locations (idClient,prix,codeSecret,fini) VALUES ("
			...
			sqlCommand = "SELECT MAX(idLocations) AS id FROM Locations;";
			requete = conn.createStatement();
			resultat = requete.executeQuery(sqlCommand);
	 ...
	 * [Sql]
	 * Met à jour la base de donnèe en créant une location associé à l'abonneId
	 * (client avec abonnement)
	 */
	public void editLocationAbonne(int idClient) {
		...
		try {
			...
			String sqlCommand = "INSERT INTO Locations (idClient,prix,codeSecret,fini) VALUES ("
	...
	/*
	 * [Sql]
	 * Intteroge la base de donnèe pour rècupèrer les valeurs de la location dont le
	 * codeSecret est codeSecret.
	 * [Java]
	 * Met à jour les valeur de l'objet courant
	 */
	...
	public void loadLocationAllVelo() {
		...
		// Sql commande
		String sqlCommand = "SELECT * FROM LocationsDetail WHERE (idLocations = " + idLocations + ");";
		try {
			requete = conn.createStatement();
			ResultSet resultat = requete.executeQuery(sqlCommand);
			while (resultat.next()) {
	...
	public int calculNbVelo() {
		...
		// Sql commande
		String sqlCommand = "SELECT COUNT(*) AS nbVelo FROM LocationsDetail WHERE (idLocations = " + this.idLocations
				+ " and fini = 0);";
		try {
			requete = conn.createStatement();
			ResultSet resultat = requete.executeQuery(sqlCommand);
	...
	public void editLocationStart(LocationsDetail sousLocation) {
		...
		try {
			...
			// Sql commande new Locations
			String sqlCommand = "UPDATE LocationsDetail SET debut = '" + sousLocation.getDebutSQL()
					+ "' WHERE (idLocationsDetail = " + sousLocation.getIdLocationsDetails() + ")";
			Statement stmt = conn.createStatement();
	...
	public void editLocationEnd(LocationsDetail sousLocation) {
		...
		try {
			...
			// Sql commande new Locations
			String sqlCommand = "UPDATE LocationsDetail SET fin = '" + sousLocation.getFinSQL()
					+ "', fini = 1 WHERE (idLocationsDetail = " + sousLocation.getIdLocationsDetails() + ")";
			Statement stmt = conn.createStatement();
	...
	public void editLocationsEnd() {
		...
		try {
			...
			// Sql commande new Locations
			String sqlCommand = "UPDATE Locations SET fini = 1 WHERE (idLocations = " + this.idLocations + ")";
			Statement stmt = conn.createStatement();
	...
	public double calculPrixLocation(LocationsDetail location) {
		...
		try {
			...
			// Sql commande new Locations
			String sqlCommand = "UPDATE LocationsDetail SET prix = " + location.getPrix()
					+ " WHERE (idLocationsDetail = " + location.getIdLocationsDetails() + ")";
			Statement stmt = conn.createStatement();
	...
	public void calculPrixLocations() {
		...
		try {
			...
			// Sql commande new Locations
			String sqlCommand = "UPDATE Locations SET prix = " + prix + " WHERE (idLocations = " + this.idLocations
					+ ");";
			Statement stmt = conn.createStatement();
